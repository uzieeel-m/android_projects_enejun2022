...
public class SpinnerClubesAdapter extends ArrayAdapter<Club> {
    ...
    private View inicializarView ( int position, @Nullable View convertView, @NonNull ViewGroup parent ) {
        ...
        if(convertView == null){
            convertView = LayoutInflater.from(getContext()).inflate(R.layout.spinner_fila_imagen_texto, parent, false);
        }
        ...
        //obtenemos el objeto club correspondiente a posicion
        Club club = getItem(position);

        //establecemos los valores del club en los widgets
